# build.ps1 - Builds libzaster.a for specified targets using Docker
...
foreach ($group in $groups) {
    ...
    foreach ($t in $group.targets) {
        $objDir = "/repo/lib/.tmp/obj/$($t.target)"
        $outDir = "/repo/prebuilt/$($t.target)"
        $outLib = "$outDir/libzaster.a"
        $srcDir = "/repo/lib"
        ...
        $ccLine = "  $($t.cc) -c $($t.cflags) -I$srcDir " + '$f -o ' + "$objDir/" + '$(basename $f .c).o'
        $lines += $ccLine
        $lines += "done"
